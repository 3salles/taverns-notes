...
test.describe('Search session on Sidebar', () => {
  test('filters the session list in real time based on the term typed.', async ({
    page,
  }) => {
    ...
    await prisma.session.createMany({
      data: [
        { title: uniqueAlpha, note: 'Alpha content' },
        { title: uniqueBeta, note: 'Beta content' },
      ],
    });
    ...
    const searchInput = page.getByPlaceholder('Buscar sessões...');
